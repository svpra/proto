    ...
    internal class ConnectedClient
    {
        ...
        private void ProcessIncomingPackets()
        {
            while (true) // Слушаем пакеты, пока клиент не отключится.
            {
                ...
                var parsed = QtPacket.Parse(buff);
        ...
        private void ProcessIncomingPacket(QtPacket packet)
        {
        ...
        private void ProcessHandshake(QtPacket packet)
        {
            Console.WriteLine("Recieved handshake packet.");

            var handshake = QtPacketConverter.Deserialize<QtPacketHandshake>(packet);
            handshake.MagicHandshakeNumber -= 15;
            
            Console.WriteLine("Answering..");

            QueuePacketSend(QtPacketConverter.Serialize(QtPacketType.Handshake, handshake).ToPacket());
        }
